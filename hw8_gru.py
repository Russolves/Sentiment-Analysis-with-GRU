...
Original file is located at
    https://colab.research.google.com/drive/1oK3_AmKZ-GpifBOuzXp3hhiNga3QrFMu
"""
...
#Define GRUnet class that uses torch.nn.GRU
class GRUnet(torch.nn.Module):
  def __init__(self, args):
      ...
      self.numLayers = 1
      self.GRU = torch.nn.GRU(self.inputSize, self.hiddenSize, self.numLayers, batch_first=True)  # Use torch.nn.GRU
...
if __name__ == '__main__':
  ...
  #Train and test torch.nn.GRU model
  args.data = 'sentiment_dataset_train_400.tar.gz'
  args.types = 'train'
  args.model = 'torchGRU'
  args.model_path = os.path.join(args.path, 'torchGRU.pth') 
  model = GRUnet(args)
  torchgrumodel, lossrecord_torchGRU = run_code_for_training(model, dataLoader, args)
  torch.save(torchgrumodel.state_dict(), args.model_path)  #Save model for torch.GRU

#Plot the figures
plt.figure()
plt.title("torchGRU Training Loss vs. Iterations")
plt.plot(lossrecord_torchGRU, label = "Training Loss")
plt.xlabel("Iterations")
plt.ylabel("Loss")
plt.legend()
# Save training loss plot
hw8_path = '/content/drive/MyDrive/BME 64600/hw8'
plt.savefig(os.path.join(hw8_path, "torchGRU_train_loss.jpg"))
plt.show()
...
args.types = 'test'
model = GRUnet(args)
accuracy_torchGRU, confusionMatrix_torchGRU = run_code_for_testing(model, dataLoader, args)

#Plot Confusion Matrix 
plotconfusionmatrix(accuracy_torchGRU, confusionMatrix_torchGRU, 'torchGRU')